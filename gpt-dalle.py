...
client = TelegramClient('bot_gpt-dalle_session_admin', config['client']['api_id'], config['client']['api_hash'])
client.start(bot_token=config['bot']['gpt-dalle-token'])

@client.on(events.NewMessage(chats=config['bot']['gpt_dalle_channels']))
